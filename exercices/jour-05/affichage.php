...
function displayPrice ($price, $discount){
    return ($discount>0)? "<s>". number_format($price, 2, ","," ") . "€ </s>" : number_format($price, 2, ","," ")."€"; 
}

function displayStock ($quantity) {
    if ($quantity===0) {
        echo "<span style=\"color: red\"> En rupture </span>";
    } else if ($quantity<10) {
        echo "<span style=\"color: orange\"> Plus que ".$quantity." articles </span>" ;
    } else {
        echo "<span style=\"color:green\"> En stock (".$quantity.")";
    }
}
    ?>
    <div style = "display : flex ; flex-wrap : wrap; gap : 5px">
        <?php foreach ($catalogue as $item):?>
            <div style = "padding : 5px; width : 200px; border:1px solid black ; border-radius : 20px ; background-color : #e9dedeff">
                <h3><?=$item["name"]?></h3>
                <?=displayBadge("A vendre", "#d1c984ff" )?>
                <p><?=$item["description"]?></p>
                <p><?=$price=displayPrice($item["prix"], $item["promo"])?></p>
                <?=($item["promo"]>0)? "<p>Nouveau prix : " . number_format($item["prix"]*(1-$item["promo"]/100), 2, ",", " ") . "€ </p>" : "";?>
                <p><?=displayStock($item["stock"])?></p>
            </div>
        <?php endforeach;?>
    </div>
